...
def on_cancel_button(widget=''):
    ...
    if listHasChanged == True :
      if tkMessageBox.askyesno(title=appName, message="Software List has changed", detail="Save the list before the application quits?", icon='warning'):
...
def renderAboutDialog():
    ...
    #aboutWindow.geometry('305x420+0+0')
...
def on_import_list(widget=''):
    ...
    if str(widget) == 'revert':
      # check if file is snet for reload
...
def writeXmlFile(xmlfilepath):
    ...
    debugPrint('[Notice] Exported list : %s' % xmlfilepath)
    appendToLog('[Notice] Exported list : %s' % xmlfilepath)
...
def on_export_list(widget=''):
    ...
    xmlFilenameName = os.path.basename(xmlFilename)
    xmlFilenameName = os.path.splitext(xmlFilenameName)[0]
    # Render Save As Dialog
    newXmlFilePath = tkFileDialog.asksaveasfilename(title='Enter Export List Name',initialdir=userHome, initialfile=xmlFilenameName, defaultextension='mai', filetypes=myfiletypes)
...
def menuControl(menuState):
    ...
    fileMenu.entryconfig("Import List", state=menuState)
...
def addItemToListDisplay(item, state):
    ...
    if state == 'add' :
      ...
      itemInfo.append(Button(frame.interior, text='i', command=lambda i=item: on_add_software(i), width=1 ))
...
def renderMainWindow():
    ...
    scaledWidth = pixelRetinaMod(615)
    ...
    mainWindow.geometry("+%d+%d" % (x-pixelRetinaMod(200), y-pixelRetinaMod(175)))
    ...
    fileMenu.add_command(label="Import List", command=on_import_list, accelerator="cmd+O")
    fileMenu.bind_all("<Command-o>", on_import_list)
    fileMenu.add_command(label="Export List", command=on_export_list, accelerator="cmd+S")
    ...
    softwareMenu.add_command(label="Add to List", command=on_add_software, accelerator="cmd++")
    softwareMenu.bind_all("<Command-=>", on_add_software)
    ...
    addButtonTxt = '+'
    addButton = Button(bottomFrame, text=addButtonTxt, command=on_add_software, width=1 )
