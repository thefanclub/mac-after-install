...
def checkInternetConnection(targetUrl):
    try:
        response=urllib2.urlopen(targetUrl, timeout=8)
    ...
    for listItem in range(itemCount):
      ...
      if itemSelected == 1 and installStateList[listItem] != 'removed':
        ...
        if os.path.exists(mountPoint) or os.path.exists(os.path.join(localDownloads,appFileList[listItem])):
          ...
          if os.path.exists(appInstallPath):
            try:
              # Set ownership to current admin user
              bashCommand="chown -R '%s' '%s'" % (os.getlogin(),appInstallPath)
              execBashCommand(bashCommand)
              # Add Admin Group
              bashCommand="chgrp admin '%s'" % appInstallPath
              execBashCommand(bashCommand)
              # Set access rights admin_user - admin - everyone
              bashCommand="chmod 755 '%s'" % appInstallPath
              execBashCommand(bashCommand)
            except:
          ...
          if os.path.exists(appBackupPath):
            try:
              ...
              execBashCommand(bashCommand)
              # Add Admin Group
              bashCommand="chgrp admin '%s'" % appBackupPath
              execBashCommand(bashCommand)
              # Set access rights admin_user - admin - everyone
              bashCommand="chmod 755 '%s'" % appBackupPath
              execBashCommand(bashCommand)
            except:
...
if __name__ == "__main__":
  ...
  if checkInternetConnection('http://www.apple.com') == False:
    # OFFLINE
    renderOfflineDialog()
